...
int main(int argc, char *argv[]) {
  ...
  addrCriteria.ai_family = AF_INET/*[YOUR CODE HERE]*/;              // Any address family
  addrCriteria.ai_flags = AI_PASSIVE;             // Accept on any address/port
  addrCriteria.ai_socktype = SOCK_DGRAM /*[YOUR CODE HERE]*/;           // Only datagram socket
  addrCriteria.ai_protocol = IPPROTO_UDP /*[YOUR CODE HERE]*/;          // Only UDP socket
  ...
  int visits = 0; // Counts the number of connections from clients
  for (;;) { // Run forever
    ...
    // [YOUR CODE HERE]  
    numBytesRcvd = recvfrom(sock, buffer, MAXSTRINGLENGTH, 0, (struct sockaddr *) &clntAddr, &clntAddrLen);
    printf("%s\n", buffer);
    if (numBytesRcvd < 0)
    ...
    // [YOUR CODE HERE]
    numBytesSent = sendto(sock, buffer, numBytesRcvd, 0, (struct sockaddr *) &clntAddr, clntAddrLen);
    if (numBytesSent < 0)
